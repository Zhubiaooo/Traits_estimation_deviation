...
for (i in Seed_source) {
  ...
  mod1 <- glmer(exist_prob ~ Field_SLA_imp_sqrt + (1|Block) , na.action=na.omit, family=binomial, data=select_data)
  ...
  mod2 <- glmer(exist_prob ~ Field_Hmax_sqrt + (1|Block) , na.action=na.omit, family=binomial, data=select_data)
  ...
  mod3 <- glmer(exist_prob ~ Field_AGB_lg + (1|Block) , na.action=na.omit, family=binomial, data=select_data)
  car::Anova(mod3)
...
mod2 = glmer(exist_prob ~ Plant_com + Pot_Hmax_sqrt + (1|Block/Plot_num) , na.action=na.omit, family=binomial, data=field_com_data)
Anova(mod2); r.squaredGLMM(mod2)[1,]

mod3 = glmer(exist_prob ~ Plant_com + Pot_AGB_lg + (1|Block/Plot_num) , na.action=na.omit, family=binomial, data=field_com_data)
Anova(mod3); r.squaredGLMM(mod3)[1,]
...
ggplot(field_com_data, aes(x= Field_AGB_lg, y=exist_prob)) +
  ...
  geom_line(data = field_com_data, mapping = aes(x=Field_AGB_lg, y=F0, color = Seed_source), size=1, linetype = 1) +
...
ggplot(field_com_data_no, aes(x=Field_SLA_imp_sqrt, y=rebio2020_100)) +
  ...
  geom_line(data = subset(field_com_data_no, Seed_source == "Shandong"), mapping = aes(x=Field_SLA_imp_sqrt, y=F0, color = Seed_source), size=1, linetype = 2) +
  geom_line(data = field_com_data_all, mapping = aes(x=Field_SLA_imp_sqrt, y=F0), size=1.5, linetype = 1, color = "black") +
  geom_ribbon(data = field_com_data_all, mapping = aes(x=Field_SLA_imp_sqrt, ymin = F0 - 1.96 * SE,ymax = F0 + 1.96 * SE, fill = "#EBEBEB"), alpha = I(0.1)) +
