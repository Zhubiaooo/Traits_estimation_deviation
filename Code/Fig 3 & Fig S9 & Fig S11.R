...
colnames(field_com_data)
select_com_data = field_com_data %>% tidyr::drop_na(rebio2022_100)
field_com_data$rebio2021_100 = log10(field_com_data$rebio2021*100)
dim(field_com_data)

mod1 = lmer(rebio2022_100 ~ Origin + (1|Block/Plot_num) , na.action=na.omit, data=field_com_data)
Anova(mod1); r.squaredGLMM(mod1)[1,]

mod2 = lmer(rebio2022_100 ~ Lifeform + (1|Block/Plot_num) , na.action=na.omit, data=field_com_data)
Anova(mod2); r.squaredGLMM(mod2)[1,]
#summary(field_com_data$rebio2022_100)
emmeans::emmeans(mod1, specs = pairwise ~ Origin, type = 'response', adjust = 'none')
Rmisc::summarySE(select_com_data, measurevar = "rebio2022_100", groupvars = "Lifeform")
library(ggpubr)
### Fig S11b
P1 = ggplot(field_com_data,aes(x=Origin,y=rebio2022_100))+
  geom_violin(data=field_com_data, aes(y=rebio2022_100,x=Origin,fill=Origin,color=Origin),trim=T,
              scale = "width",position = position_dodge(0.5),width=0.6,alpha = 0.8, size = 1.5)+
  geom_boxplot(data=field_com_data, aes(y=rebio2022_100,x=Origin),fill="white",color="black",
               position = position_dodge(0.5),width=0.15,outlier.size=0.8,outlier.shape = 1)+
  stat_summary(data=field_com_data, aes(y=rebio2022_100,x=Origin),fill="white",color="black",
...
P2 <- ggplot(dat, aes(variable, value, fill = Type)) +
  ...
  scale_fill_manual(values = c('#6A4939','#3AB3A8'),
...
colnames(field_com_data)
mod1 = lmer(rebio2022_100 ~ Field_SLA_imp_sqrt + (1|Block/Plot_num), data=select_data)
mod1 = lmer(rebio2022_100 ~ Field_SLA_imp_sqrt + (1|Block/Plot_num), na.action=na.omit, data = field_com_data)
Anova(mod1); r.squaredGLMM(mod1)[1,]

mod2 = lmer(rebio2022_100 ~ Field_Hmax_sqrt + (1|Block/Plot_num), data=select_data)
Anova(mod2); r.squaredGLMM(mod2)[1,]

mod3 = lmer(rebio2022_100 ~ Field_AGB_lg + (1|Block/Plot_num), data=select_data)
Anova(mod3); r.squaredGLMM(mod3)[1,]
...
colnames(field_com_data)
mod1 = lmer(rebio2022_100 ~ Pot_SLA_sqrt + (1|Block/Plot_num), data=select_data)
Anova(mod1); r.squaredGLMM(mod1)[1,]

mod2 = lmer(rebio2022_100 ~ Pot_Hmax_sqrt + (1|Block/Plot_num), data=select_data)
Anova(mod2); r.squaredGLMM(mod2)[1,]

mod3 = lmer(rebio2022_100 ~ Pot_AGB_lg + (1|Block/Plot_num), data=select_data)
Anova(mod3); r.squaredGLMM(mod3)[1,]
...
################################################################################
#### SLA
mod12 <- lmer(rebio2022_100 ~ Pot_SLA_sqrt*Seed_source + (1|Block), data = field_com_data_no)
summary(mod12)
anova(mod12)
MuMIn::r.squaredGLMM(mod12)
field_com_data_no$F0 = predictSE(mod12, field_com_data_no, level = 0)$fit
