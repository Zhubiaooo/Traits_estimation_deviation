library(openxlsx)
library(dplyr)
com_data <- read.xlsx("Data/Common_species_list.xlsx", sheet = "Common_sp_list", colNames = TRUE, rowNames = FALSE)
com_sp = unique(com_data$Species); com_sp
###############################################################################
#### 仅对田间实验中比叶面积进行统计
field_row_data <- read.xlsx("Data/all_row_data0829.xlsx", sheet = "Field_data_row_mean", rowNames = F, colNames = T) %>%
  tidyr::drop_na(SLA) #### 移除缺失值
head(field_row_data)
unique(field_row_data$Species)

#### 移除某些物种某个种群观测值小于3的数据
summary_sp_data <- (field_row_data[,c("Species","Seed_source")])
...
field_row_data = field_row_data %>% left_join(summary_sp_data[,c("Species","Seed_source","N")], by = c("Species","Seed_source"))
field_row_data = field_row_data %>% tidyr::drop_na(N)
unique(field_row_data$Species)
#### 按照以block为单位，分别计算各个物种性状均值
head(field_row_data)
SLA_mean_ind = field_row_data %>% group_by(Block, Species,`物种名`,Origin, Lifeform) %>% summarise(SLA = mean(SLA))
unique(SLA_mean_ind$Species)
#### 按照以Species为单位，分别计算各个物种性状均值
SLA_mean_sp = Rmisc::summarySE(SLA_mean_ind, groupvars = "Species", measurevar = "SLA")

#### 仅对田间实验中株高、地上生物量进行统计
field_row_data <- read.xlsx("Data/all_row_data0829.xlsx", sheet = "Field_data_row_mean", rowNames = F, colNames = T) %>%
  tidyr::drop_na(AGB) #### 移除缺失值
head(field_row_data)
unique(field_row_data$Species)
...
#### 按照以block为单位，分别计算各个物种性状均值
head(field_row_data)
Hmax_AGB_mean_ind = field_row_data %>% group_by(Block, Species,`物种名`,Origin, Lifeform) %>% 
  summarise(Hmax = mean(Hmax), AGB = mean(AGB))
unique(Hmax_AGB_mean_ind$Species)

#### 按照以Species为单位，分别计算各个物种性状均值
Hmax_mean_sp = Rmisc::summarySE(Hmax_AGB_mean_ind, groupvars = "Species", measurevar = "Hmax")
AGB_mean_sp = Rmisc::summarySE(Hmax_AGB_mean_ind, groupvars = "Species", measurevar = "AGB")

#### 按照以Species为单位，三个性状均值新数据集
Field_mean_data = merge(Hmax_mean_sp[,c("Species", "Hmax")],
                        AGB_mean_sp[,c("Species", "AGB")], by = "Species") %>% 
  left_join(SLA_mean_sp[,c("Species", "SLA")], by = "Species")

Field_mean_Imput = Field_mean_data %>% left_join(com_data[, c("Species","Origin")], by = c("Species"))
Field_mean_Imput$Origin = NULL
# write.xlsx(Field_mean_Imput, "Data/Imputation_data0901.xlsx", sheetName = "Imputation_data")
